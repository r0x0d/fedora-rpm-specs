...
%global outdir out/Release_GN_%{chromium_arch}
...
Patch420:	syscall-broker-aarch64-f41.patch
Patch421:	flatbuffers-const-member.patch
...
# Security fixes (CVE-2023-6349)
Patch600:	libvpx-vp9_alloccommon-clear-allocation-sizes-on-free.patch
Patch601:	libvpx-Fix-bug-with-smaller-width-bigger-size.patch
...
%patch 420 -p1 -b .aarch64-syscalls
%patch 421 -p1 -b .const
...
%patch 600 -p1 -b .libvpx-cve1
%patch 601 -p1 -b .libvpx-cve2
...
mkdir -p %{outdir} && cp -a %{_bindir}/gn %{outdir}/

mkdir -p %{outdir}/pycompat
# buildsystem uses pipes.quote all over the place. Add a dummy implementation for py3.13 compat.
echo 'from shlex import quote' > %{outdir}/pycompat/pipes.py

PYTHONPATH=%{outdir}/depot_tools:%{outdir}/pycompat \
...
export PYTHONPATH="../../third_party/pyjson5/src:../../xcb-proto-1.14:../../third_party/catapult/third_party/html5lib-1.1:$PWD/%{outdir}/pycompat"

echo

# Missing build deps?
%build_target %{outdir} includes/include/cef_config.h

%build_target %{outdir} cefclient
%build_target %{outdir} chrome_sandbox

# bug #827861, vk_swiftshader_icd.json not getting properly installed in out/Release
sed -e 's|${ICD_LIBRARY_PATH}|./libvk_swiftshader.so|g' third_party/swiftshader/src/Vulkan/vk_swiftshader_icd.json.tmpl > %{outdir}/vk_swiftshader_icd.json

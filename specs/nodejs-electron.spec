## ELECTRON README
# This spec file is based on chromium.spec, as Electron is based on
# Chromium. To keep things maintainable, the Electron version attempts
# to track the chromium version released in Fedora where possible, and
# the changes in this file are bracketed in `## ELECTRON` and `## END
# ELECTRON` comments. Updates to the underlying Chromium version can
# be achieved by merging in the changes to chromium.spec, or by
# manually re-applying the Electron changes to the new Chromium spec.
## END ELECTRON
...
%define abi_version 148
%global electron_path %{_libdir}/electron
## END ELECTRON
...
## ELECTRON
# Electron has not been ported to GTK4; build with GTK3.
# https://github.com/electron/electron/issues/33690
# https://github.com/electron/electron/issues/48580
...
## ELECTRON
# To generate this list, go into %%{buildroot}%%{electron_path} and run
...
%global __provides_exclude_from ^(%{electron_path}/.*\\.so|%{electron_path}/.*\\.so.*)$
%global __requires_exclude ^(%{electron_path}/.*\\.so|%{electron_path}/.*\\.so.*)$
## END ELECTRON
...
# Chromium version pinned by Electron DEPS
%global chromium_version 150.0.7871.129
## END ELECTRON
...
## ELECTRON: patches
Patch700: system-zstd-in-node.patch
Patch701: system-ffmpeg.patch
Patch702: reduce-gn-tree.patch
## END ELECTRON
...
## ELECTRON: Use %%{chromium_version} for tarball name
...
Source20: electron-%{electron_version}.tar.zst
Source21: electron-sources.sh
# RPM macros for building electron native module packages (installed in %%install)
Source22: macros.electron
## END ELECTRON
...
# Without full ICU data, electron-typescript-definitions crashes:
...
Provides: electron%{_isa} = %{version}
...
## ELECTRON: Package description and subpackages
%description
Electron is a framework for building cross-platform desktop applications
using JavaScript, HTML, and CSS. It embeds Chromium for rendering and
Node.js for backend functionality.

%package -n nodejs-electron-devel
Summary: Electron development headers and macros
Requires: nodejs-electron%{_isa} = %{version}-%{release}
Requires: /usr/bin/npm

%description -n nodejs-electron-devel
Development headers for building native Node.js modules against Electron.

%if %{use_qt5}
%package qt5-ui
Summary: Qt5 UI for Electron
Requires: nodejs-electron%{_isa} = %{version}-%{release}
Supplements: (nodejs-electron%{_isa} and qt5-qtbase-gui%{_isa})

%description qt5-ui
Qt5 UI integration for Electron.
%endif
...
Requires: nodejs-electron%{_isa} = %{version}-%{release}
Supplements: (nodejs-electron%{_isa} and qt6-qtbase-gui%{_isa})

%description qt6-ui
Qt6 UI integration for Electron.
%endif
## END ELECTRON

%prep
## ELECTRON: Extract Electron overlay on top of Chromium source
%setup -q -n chromium-%{chromium_version}
tar xf %{SOURCE20}

# Check that abi_version macro matches the actual ABI
test $(grep ^node_module_version electron/build/args/all.gn | sed 's/.* = //') = %abi_version

# Create v8 symlinks needed by electron_node
ln -srv v8/tools third_party/electron_node/deps/v8/tools
ln -srv v8/include third_party/electron_node/deps/v8/include

# config.json paths are relative to a parent dir with src/ prefix
ln -s . src

# Apply Electron's patches to the Chromium tree (before Fedora patches).
# apply_all_patches.py uses git am, so each target needs a git repo.
export GIT_COMMITTER_NAME="RPM Build" GIT_COMMITTER_EMAIL="rpm@build"
export GIT_AUTHOR_NAME="RPM Build" GIT_AUTHOR_EMAIL="rpm@build"
for repo in . third_party/boringssl/src third_party/devtools-frontend/src \
            third_party/ffmpeg v8 third_party/electron_node third_party/nan \
            third_party/perfetto third_party/webrtc third_party/sqlite/src; do
    if [ -d "$repo" ]; then
        git init "$repo"
        git -C "$repo" add -A
...
## ELECTRON: patches
%patch -P700 -p1 -b .system-zstd-in-node
%patch -P701 -p1 -b .system-ffmpeg
%patch -P702 -p1 -b .reduce-gn-tree
## END ELECTRON
...
## ELECTRON
# enable_enterprise_companion is not defined in Electron's GN scope;
# setting it produces a warning that breaks gn desc output.
## END ELECTRON
...
## ELECTRON: GN defines and build targets ###
ELECTRON_GN_DEFINES=""
ELECTRON_GN_DEFINES+=' override_electron_version="%{electron_version}"'
ELECTRON_GN_DEFINES+=' use_v8_context_snapshot=true'
ELECTRON_GN_DEFINES+=' v8_use_external_startup_data=true'
ELECTRON_GN_DEFINES+=' v8_enable_sandbox=false'
ELECTRON_GN_DEFINES+=' enable_screen_ai_service=false'
ELECTRON_GN_DEFINES+=' enable_screen_ai_browsertests=false'
ELECTRON_GN_DEFINES+=' use_fake_screen_ai=true'
ELECTRON_GN_DEFINES+=' enable_widevine=false'
ELECTRON_GN_DEFINES+=' v8_builtins_profiling_log_file=""'
ELECTRON_GN_DEFINES+=' angle_use_custom_libvulkan=false'

export ELECTRON_OUT_DIR=Release

%{chromebuilddir}/gn --script-executable=%{chromium_pybin} gen \
    ...
    --args="import(\"//electron/build/args/release.gn\") $CHROMIUM_CORE_GN_DEFINES $CHROMIUM_BROWSER_GN_DEFINES $ELECTRON_GN_DEFINES" \
    %{chromebuilddir}

%build_target %{chromebuilddir} v8_context_snapshot_generator
%build_target %{chromebuilddir} chromium_licenses
%build_target %{chromebuilddir} copy_node_headers
%build_target %{chromebuilddir} version
%build_target %{chromebuilddir} electron
## END ELECTRON
...
mkdir -p %{buildroot}%{electron_path}
mkdir -p %{buildroot}%{_includedir}/electron

# Install electron binary and runtime deps via gn desc
pushd %{chromebuilddir}
install -pm 0644 version -t %{buildroot}%{electron_path}/

# gn desc may emit warnings to stdout; filter to paths only (no spaces)
gn desc \
    --root-target=//electron:electron_app \
    --root-pattern=//electron:electron_app \
    --root-pattern=//electron:chromium_licenses \
    --root-pattern=//electron:copy_node_headers \
    --root-pattern=//electron:electron_version_file \
    . //electron:electron_app runtime_deps \
    | grep -v ' ' | grep -v ^gen/ | sort -u \
    | xargs cp -a --parents -t %{buildroot}%{electron_path}/ --
popd

# Install node headers for native module development
cp -lrvT %{chromebuilddir}/gen/node_headers %{buildroot}%{_includedir}/electron

# Launcher script
cat > %{buildroot}%{_bindir}/electron << 'EOF'
#!/bin/sh
exec %{electron_path}/electron "$@"
EOF
chmod 0755 %{buildroot}%{_bindir}/electron

# electron-node wrappers for native module rebuilds
mkdir -p %{buildroot}%{_libexecdir}/electron-node

cat > %{buildroot}%{_libexecdir}/electron-node/node << 'EOF'
#!/bin/sh
ELECTRON_RUN_AS_NODE=1 exec %{electron_path}/electron "$@"
EOF

cat > %{buildroot}%{_libexecdir}/electron-node/npm << 'EOF'
#!/bin/sh
exec %{_libexecdir}/electron-node/node %{_bindir}/npm "$@"
EOF

cat > %{buildroot}%{_libexecdir}/electron-node/npx << 'EOF'
#!/bin/sh
exec %{_libexecdir}/electron-node/node %{_bindir}/npx "$@"
EOF

cat > %{buildroot}%{_libexecdir}/electron-node/yarn << 'EOF'
#!/bin/sh
exec %{_libexecdir}/electron-node/node %{_bindir}/yarn "$@"
EOF

chmod 0755 %{buildroot}%{_libexecdir}/electron-node/*

# RPM macros for electron native module packages (see Source22: macros.electron).
# Bake in the macros that describe files provided by this electron build; leave
# %%{buildroot} literal so it resolves against the consuming package's tree.
mkdir -p %{buildroot}%{_rpmmacrodir}
sed -e 's|%%{abi_version}|%{abi_version}|g' \
    -e 's|%%{_isa}|%{_isa}|g' \
    -e 's|%%{_libexecdir}|%{_libexecdir}|g' \
    -e 's|%%{_includedir}|%{_includedir}|g' \
    -e 's|%%{_libdir}|%{_libdir}|g' \
    %{SOURCE22} > %{buildroot}%{_rpmmacrodir}/macros.electron
chmod 0644 %{buildroot}%{_rpmmacrodir}/macros.electron
## END ELECTRON
...
%license LICENSE electron/LICENSE %{chromebuilddir}/LICENSES.chromium.html
%{_bindir}/electron
%dir %{electron_path}/
%{electron_path}/electron
%{electron_path}/version
%{electron_path}/chrome_100_percent.pak
%{electron_path}/chrome_200_percent.pak
%{electron_path}/chrome_crashpad_handler
%{electron_path}/resources.pak
%{electron_path}/libEGL.so
%{electron_path}/libGLESv2.so
%{electron_path}/libvk_swiftshader.so
%{electron_path}/libvulkan.so.1
%{electron_path}/vk_swiftshader_icd.json
%{electron_path}/snapshot_blob.bin
%{electron_path}/v8_context_snapshot.bin
%if %{bundleicu}
%{electron_path}/icudtl.dat
%endif
%dir %{electron_path}/resources/
%{electron_path}/resources/default_app.asar
%dir %{electron_path}/locales/
%{electron_path}/locales/*.pak
%{electron_path}/pyproto/

%files -n nodejs-electron-devel
%{_includedir}/electron/
%{_rpmmacrodir}/macros.electron
%dir %{_libexecdir}/electron-node/
%{_libexecdir}/electron-node/*

%if %{use_qt5}
%files qt5-ui
%{electron_path}/libqt5_shim.so
%endif

%if %{use_qt6}
%files qt6-ui
%{electron_path}/libqt6_shim.so
%endif
## END ELECTRON

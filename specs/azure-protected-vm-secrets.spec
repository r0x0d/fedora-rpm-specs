...
# release tag azure-protected-vm-secrets-v%%{version} by the script in Source10.
...
# It is extracted in the script from Source10
Source1:        LICENSE

# Script to generate the tarball in Source0 from the upstream workspace repository.
Source10:       generate-source-tarball.sh
...
# The README contains CRLF line endings
dos2unix README.md
...
# subtree), so it is shipped as Source1 and staged into the build dir so
# %%license can reference it below. -p preserves the original timestamp.
install -D -p -m 0644 %{SOURCE1} %{_builddir}/%{buildsubdir}/LICENSE
# Install the CLI man page (RPM compresses it automatically).
install -D -p -m 0644 packaging/fedora/azure-protected-secrets-tool.1 %{buildroot}%{_mandir}/man1/azure-protected-secrets-tool.1

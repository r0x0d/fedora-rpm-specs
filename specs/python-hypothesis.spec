...
%if %{with tests}
# Test dependencies are spread across multiple files with
# `-r` style links between them, so it's easier to list the
# required test dependencies here manually.
BuildRequires:  %{py3_dist attrs}
BuildRequires:  %{py3_dist pexpect}
BuildRequires:  %{py3_dist pytest}
BuildRequires:  %{py3_dist pytest-xdist}
BuildRequires:  %{py3_dist watchdog}
...
%if %{with tests}
%check
# test_registered_from_entrypoint requires an editable installation of an example package
# `example_hypothesis_entrypoint`. We don't do that in the RPM build, hence skipping.

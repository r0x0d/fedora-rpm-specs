# Generated by rust2rpm 27
%bcond check 1

%global crate startup-disk
%global app_id org.startup_disk.StartupDisk

Name:           rust-startup-disk
Version:        0.1.5
Release:        %autorelease
Summary:        Interface to choose the startup volume on Apple Silicon systems

License:        MIT
URL:            https://crates.io/crates/startup-disk
Source:         %{crates_source}
# Manually created patch for downstream crate metadata changes
# * bump glib-build-tools dependency to 0.20 to match libadwaita dependency
Patch:          startup-disk-fix-metadata.diff

BuildRequires:  cargo-rpm-macros >= 24
BuildRequires:  make
%if %{with check}
BuildRequires:  desktop-file-utils
BuildRequires:  libappstream-glib
%endif

%global _description %{expand:
Startup Disk provides a simple interface to choose the startup volume on Apple
Silicon Macs running Asahi Linux.}

%description %{_description}

%package     -n %{crate}
Summary:        %{summary}
# Apache-2.0 OR MIT
# BSD-2-Clause OR Apache-2.0 OR MIT
# MIT
# MIT OR Apache-2.0
# Unlicense OR MIT
# Zlib
License:        (Apache-2.0 OR MIT) AND (BSD-2-Clause OR Apache-2.0 OR MIT) AND MIT AND (MIT OR Apache-2.0) AND (Unlicense OR MIT) AND Zlib
# LICENSE.dependencies contains a full license breakdown
Requires:       hicolor-icon-theme
Requires:       polkit
Requires:       sudo

%description -n %{crate} %{_description}

%files       -n %{crate}
%license LICENSE
%license LICENSE.dependencies
%doc README.md
%{_bindir}/startup-disk
%{_datadir}/applications/%{app_id}.desktop
%{_datadir}/icons/hicolor/scalable/apps/%{app_id}.svg
%{_datadir}/polkit-1/actions/%{app_id}.policy
%{_metainfodir}/%{app_id}.metainfo.xml

%prep
%autosetup -n %{crate}-%{version} -p1
%cargo_prep

%generate_buildrequires
%cargo_generate_buildrequires

%build
%cargo_build
%{cargo_license_summary}
%{cargo_license} > LICENSE.dependencies

%install
%cargo_install
make install-data DESTDIR="%{buildroot}" DATADIR="%{_datadir}"

%if %{with check}
%check
%cargo_test
make check-data
%endif

%changelog
%autochangelog

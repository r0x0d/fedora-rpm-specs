...
BuildOption(install):   -l strictyaml
# There are no tests. (If there were, it would be as hard to run them as it is
# to build the documentation.) We must skip one module in the bundled
# ruamel-yaml because it does not include the compiled C extension
# (_ruamel_yaml).

...
%define regenerate_shas() %{expand:
# regenerate SHAs to make jmod-less image work;
%{sdkbindir -- %{?1}}/java --add-opens=jdk.jlink/jdk.tools.jlink.internal.runtimelink=ALL-UNNAMED %{_jvmdir}/%{sdkdir -- %{?1}}/lib/BuildShaSumFile.java %{_jvmdir}/%{sdkdir -- %{?1}}/conf/runtimelink-sha-overrides.conf  > /dev/null 2>&1
}


%define posttrans_devel() %{expand:
%{update_desktop_icons}
# maybe it would be better to do in headless?
%{regenerate_shas -- %{?1}}
}
...
# This is regenerated by posttrans thus not config, nor even file? ghost?
%{etcjavadir -- %{?1}}/conf/runtimelink-sha-overrides.conf
# this regenerates ^
%{_jvmdir}/%{sdkdir -- %{?1}}/lib/BuildShaSumFile.java
...
function installjdk() {
    ...
    if [ -d ${imagepath} ] ; then
        ...
        # create jlink fiel so it can be tracked and reused
        create_jlink_argfile ${imagepath}
        # include file capable to regenerate images in posttrans
        # this terribel approach have only one advantage - it works even with properly stripped debuginfo
        cp %{SOURCE19} ${imagepath}/lib/`basename %{SOURCE19}`

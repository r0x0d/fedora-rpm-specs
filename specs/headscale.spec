...
# MIT
Provides:       bundled(golang(github.com/arl/statsviz)) = v0.8.0
# MIT
...
# MIT
Provides:       bundled(golang(github.com/axiomhq/hyperloglog)) = v0.2.6
# MIT
...
# MIT
Provides:       bundled(golang(github.com/dgryski/go-metro)) = v0.0.0~20250106013310~edb8663e5e33
# Apache-2.0
...
# not look functional. So we patch the values in via bash/sed for now.
...
# Using -E for extended regex (so we don't have to escape parentheses)
sed -i.bak -E \
    -e "s|(Version:[[:space:]]+)\"dev\"|\1\"%{version}\"|g" \
    -e "s|(Commit:[[:space:]]+)\"unknown\"|\1\"v%{version}\"|g" \
    -e "s|(BuildTime:[[:space:]]+)\"unknown\"|\1\"${BUILD_DATE}\"|g" \
    "${version_file}"

diff -urN "${version_file}"{.bak,} ||:
rm -f "${version_file}.bak"

# non-functional ldflags to set version/build time
# Capture current time in ISO 8601 format
#export BUILD_TIME=$(date -u +%%Y-%%m-%%dT%%H:%%M:%%SZ)
#export LDFLAGS="-X %{goipath}/cmd/headscale/cli.Version=v%{version} \
#                -X %{goipath}/hscontrol/types.BuildTime=${BUILD_TIME}"
...
./bin/headscale version | grep -q "version %{version}" && \
./bin/headscale version | grep -q "commit: v%{version}" || exit 1
...
* Tue Feb 03 2026 Maxwell G <maxwell@gtmx.me> - 0.26.1-8
...
* Thu Jul 24 2025 Fedora Release Engineering <releng@fedoraproject.org> - 0.26.1-4
